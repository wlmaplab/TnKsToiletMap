...
class ToiletMapView: MKMapView, MKMapViewDelegate {
    ...
    //台南火車站
    let tainanSiteCoordinate = CLLocationCoordinate2D(latitude: 22.99736071185278,
                                                      longitude: 120.21254476334039)
    ...
    func moveToSite() {
        let region = MKCoordinateRegion(center: tainanSiteCoordinate,
    ...
    func addToiletData(_ array: [Toilet]) {
        ...
        for item in array {
            let image: UIImage?
            if item.city == "tn" {
                image = UIImage(named: "TnToiletPin")
            } else {
                image = UIImage(named: "KsToiletPin")
            }
